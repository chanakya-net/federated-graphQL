...
| 2026-09-18 | P6 | phase-6 §4 helpers: `GW=...:${GATEWAY_PORT:-5000}`, `gql` builds `auth=()` and expands `"${auth[@]}"` under `set -u`, "bash 3.2+" | Port 5000 is taken on macOS (P0/P1 rows). Under `set -u`, bash 3.2 (macOS `/bin/bash` 3.2.57) fails on an empty array with `a[@]: unbound variable`, i.e. on the unauthenticated call (checked) | `scripts/e2e.sh` reads `GATEWAY_PORT` / `UI_PORT` / `SUBGRAPH_TIMEOUT_SECONDS` from the shell, else `.env`, else 5050 / 4200 / 5 (same as `up.sh`), and expands `${auth[@]+"${auth[@]}"}`. Run and passing under `/bin/bash` 3.2.57 |
| 2026-09-18 | P6 | phase-6 §4 scenario 20: `patchEvents(since: <epoch-30d>, until: <epoch>)`, assert "all `occurredAt` within bounds" | `since` / `until` are `DateTime` (ISO 8601), not epoch numbers. `dev-00001` has **no** patch event in `[Epoch - 30 d, Epoch]`, so the assertion passes on an empty list and proves nothing | Window `[Epoch - 180 d, Epoch - 30 d]` (from `SeedConstants.Epoch`, computed with jq). For all three fields: every event is inside the bounds, and the ids equal the unfiltered list filtered client-side (non-empty: 3 / 9 / 8). The same with the bounds written as `+05:30` offsets (the P3B UTC bug class), and a one-instant window (`since == until`) returns the event at that instant (inclusive bounds) |
| 2026-09-18 | P6 | phase-6 §4 scenario 18: `devices(search:"dev-00", first: 5)` as dave, every `items[].tenantId == "TenantB"` | TenantB ids are `dev-07000`..`dev-11999`, so dave's `dev-00` search returns 0 items and the check passes on an empty list | Dave `dev-0`: `totalCount == 3000`, 5 items, all TenantB. Dave `dev-00`: `totalCount == 0` (TenantA ids invisible). Alice `dev-0`: `totalCount == 7000`, all TenantA |
| 2026-09-18 | P6 | phase-6 §4: `trap restore_all EXIT` restores all four services; scenario 11 "as scenario 1" | Restoring a service the run never touched is harmless but slow, and the script is meant to leave the stack as it found it | A preflight requires all nine long-running services healthy (`E2E_WAIT_TIMEOUT`, default 60 s) and five tokens in `/tokens.json` (exit 2 otherwise). The EXIT trap restores only the services this run stopped or paused (checked: a forced failure while `patch` was paused left `patch` healthy). Each scenario also checks the HTTP status. Scenarios 14/15 check degraded **and** restored. Scenario 7 also requires the two `AUTH_NOT_AUTHORIZED` errors. Scenario 8 also sends a tampered token (401, empty body) |
| 2026-09-18 | P6 | phase-6 §5 / §6 step 2: Nitro "query plan view shows the fan-out"; P4 `Program.cs`: `CollectOperationPlanTelemetry = true; // query plan view in Nitro` | Nitro asks for the plan with the request header `Fusion-Operation-Plan: 1` (in `ChilliCream.Nitro.App`; `HotChocolate.AspNetCore` `HttpHeaderKeys.OperationPlan`). The gateway returns `extensions.fusion.operationPlan` only when `FusionRequestOptions.AllowOperationPlanRequests` is true, which is **false by default** in 16.6.6. Checked: without it, the header gives no plan | **P6 edited `src/Gateway/Program.cs` (P4-owned)**: `AllowOperationPlanRequests = true`. Gateway unit tests 34/34. The plan now shows `DeviceDirectory` (no dependencies), then `Patch`, `Vulnerability`, `SoftwareInstall`, each depending only on it. New e2e scenario `query_plan_fans_out` (21 in total) checks this. Trade-off: any authenticated caller can read the plan (subgraph names, internal URLs, variables). Fine for this dev-only POC. Turn it off anywhere real |
| 2026-09-18 | P6 | phase-6 §4 scenario 16: "record which code appears" | No code: `{"message":"Unexpected Execution Error","path":["device","installEvents"]}`, the outage shape (P5 row). Scenario 17: HTTP **200** with `device: null` and one error while Device Directory is stopped | Recorded in the scenario output (`code: none`). The UI shows bob's section as **unavailable** while Software Install is down, which is the correct text: nothing could decide a denial |
